...
const WORLD = [
  "Чёрный Пояс не спит. Он притворяется мёртвым. Заводы давно погасли, но под асфальтом ещё гудит тепло.",
  "Говорят, в старых тоннелях есть круг, куда не пускают просто так. «Колесница войны». Имя как шутка — пока не увидишь ворота.",
  "Это не чемпионат. Пушки на капоте. Хлам вместо кузова. Финиш — когда ты ещё дышишь, а кто-то уже нет.",
  "Наверху ставят деньги на то, как красиво ты сгоришь. Военные делают вид, что этого места нет.",
  "Арена читает не скорость. Страх. Долги. Того, кого ты не сможешь добить.",
  "Пропуск приходит без отправителя. «Участие добровольное». Ворота закрываются за спиной.",
  "Свет на решётке. Мотор уже тёплый. Садись. Газ. Посмотрим, зачем тебя сюда пустили."
];
...
const PILOTS = [
  {
    ...
    car: "assets/data/cars/01/01.webp",
    frames: 7,
  ...
  },
  {
    ...
    name: "Ерш",
    role: "Сын Башкира",
    car: "assets/data/cars/08/08.webp",
    frames: 6,
  ...
  },
  {
    ...
    name: "Бегемотик",
    role: "Жена Ерша",
    car: "assets/data/cars/09/09.webp",
    frames: 6,
  ...
  },
  {
    ...
    name: "Башкир",
    role: "Отец Ерша",
    car: "assets/data/cars/07/07.webp",
    frames: 7,
  ...
  },
  {
    ...
    car: "assets/data/cars/11/11.webp",
    frames: 7,
...
function setupAgeGate() {
  ...
  if (!layer) return;
  if (sessionStorage.getItem("kv-age") === "1") return;
  layer.hidden = false;
...
function setupWorld() {
  ...
  const paint = (next) => {
    ...
    art.src = `assets/data/cats/00/0${index + 1}.webp`;
    text.textContent = WORLD[index];

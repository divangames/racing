  ...
  function placeTrackHazardsEngine(T, seed) {
    function mergeGapRamps(ramps) {
      ...
      for (let i = 0; i < extra.length; i++) {
        const rp = extra[i];
        if (ramps.every(function (q) { return Math.min(Math.abs(q.i - rp.i), NN - Math.abs(q.i - rp.i)) > 12; })) {
          ramps.push(rp);
        } else {
          for (let j = 0; j < ramps.length; j++) {
            if (Math.min(Math.abs(ramps[j].i - rp.i), NN - Math.abs(ramps[j].i - rp.i)) <= 12) {
              ramps[j].gap = true;
              ramps[j].boost = rp.boost;
              break;
            }
          }
        }
    ...
    for (let j = 0; j < 8 && mid.length; j++) {
      ...
      if (mines.every(function (m) { return Math.abs(m.i - o.i) > 26; }) && notGap(o.i)) mines.push({ i: o.i, lat: (Rz() * 2 - 1) * (ROADW - 40) });
    }
    for (let j = 0; j < 6; j++) {
      const i = 80 + ((Rz() * (N - 160)) | 0);
      if (oils.every(function (o) { return Math.abs(o.i - i) > 30; }) && notGap(i)) oils.push({ i: i, lat: (Rz() * 2 - 1) * (ROADW - 36), rot: Rz() * TAU });
    }
    ...
    for (let j = 0; j < types.length; j++) {
      ...
      while ((picks.some(function (p) { return Math.abs(p.i - i) < 16; }) || !notGap(i)) && tr++ < 20) i = 60 + ((Rz() * (N - 120)) | 0);

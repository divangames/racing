...
/**
 * Панорама в осях экрана: поворот машины не переворачивает левый и правый каналы.
 */
function carEnginePanFrom(listener, racer) {
  if (!listener || !racer) return 0;
  const dx = racer.x - listener.x;
  return Math.max(-.82, Math.min(.82, dx / (CAR_ENGINE_PAN * 1.5)));
}

...
function carEngineVol() {
  ...
  const level = Math.max(0, Math.min(1, (settings.sound.sfx ?? 80) / 100));
  if (!level) return 0;
  return (window.DiVANEngine && DiVANEngine.audioMix && AU.ctx ? 1 : level) * CAR_ENGINE_WAV;
}
...
function carEngineDrainHtmlPool(slot) {
  ...
  for (const key in pool) {
    const el = pool[key];
    if (window.DiVANEngine && DiVANEngine.audioMix) DiVANEngine.audioMix.releaseMedia(el);
...
function carEngineTouchSlot(slot, vol, rate) {
  ...
  if (v.el) {
    ...
    if (window.DiVANEngine && DiVANEngine.audioMix) DiVANEngine.audioMix.routeMedia(v.el, slot.pan || 0);
    if (pitch) {
  ...
  if (v.gain) {
    try {
      if (window.DiVANEngine && DiVANEngine.audioMix) DiVANEngine.audioMix.smooth(v.gain.gain, vol);
      else v.gain.gain.value = vol;
    } catch (err) {}
  }
  ...
  if (v.pan) {
    try {
      if (window.DiVANEngine && DiVANEngine.audioMix) DiVANEngine.audioMix.smooth(v.pan.pan, slot.pan || 0, .065);
      else v.pan.pan.value = slot.pan || 0;
    } catch (err) {}
  }
}
...
function carEnginePlayHtml(slot, url, loop, vol, rate, force) {
  ...
  el.loop = false;
  el.volume = loud;
  if (window.DiVANEngine && DiVANEngine.audioMix) DiVANEngine.audioMix.routeMedia(el, slot.pan || 0);
...
function carEngineHtmlSeam(slot, fromEnded) {
  ...
  if (window.DiVANEngine && DiVANEngine.audioMix) DiVANEngine.audioMix.routeMedia(twin, slot.pan || 0);
...
function carEnginePlayWeb(slot, url, buf, loop, vol, rate, pan, force) {
  ...
  if (!force && cur && cur.url === url && cur.loop === loop && cur.src && !cur.el) {
    carEngineTouchSlot(slot, vol, pitch);
    return true;
  }

...
const EditorApp = (() => {
  ...
  const diskStates = new Map();

  /** Сериализует записи одного слота: старый запрос не перезапишет новые правки. */
  function writeSlot(slot, value, kind) {
    const saved = EditorData.clone(value);
    const previous = diskWrites.get(slot) || Promise.resolve();
    const task = previous.then(() => kind === 'base'
      ? EditorData.saveAsBase(slot, saved) : EditorData.pushDisk(slot, saved)).catch(() => false);
    diskWrites.set(slot, task);
    diskStates.set(slot, 'saving');
    task.then(ok => {
      if (diskWrites.get(slot) !== task) return;
      diskWrites.delete(slot);
      diskStates.set(slot, ok ? 'saved' : 'failed');
      if (slot === carIndex && JSON.stringify(carForHistory(car())) === JSON.stringify(carForHistory(saved))) {
        dirty = !ok;
        setText('saveState', ok ? 'Изменения записаны в файл' : 'Файл не записан · черновик сохранён локально');
      }
    });
    return task;
  }
  ...
  /** Не записывает незавершённый жест поверх предыдущего сохранённого шага. */
  function beginEdit() {
    flushCommit();
    clearTimeout(persistTimer); persistTimer = null;
  }

  /** После отмены жеста сохраняет предшествующий ему подтверждённый шаг. */
  function resumeSave() { if (dirty) persistSoon(); }
  ...
  function undo() {
    if (window.CarLightEditor) CarLightEditor.finish();
  ...
  async function saveSettings() {
    ...
    flushCommit();
    clearTimeout(persistTimer); persistTimer = null;
    const c = car(), slot = carIndex, serial = editSerial;
    c.rev = Date.now();
    setText('saveState', 'Записываю базу и рабочий файл…');
    try {
      EditorData.save(pack);
      const ok = await writeSlot(slot, c, 'base');
      if (slot !== carIndex || serial !== editSerial) return ok;
      if (ok) {
        dirty = false;
        ...
        dirty = true;
        setText('saveState', 'Не удалось записать файл · черновик сохранён локально');
        showFileNote(false, 'Сохранение', 'Файлы не записались',
          'Правки остались в локальном черновике. Проверьте доступ к папке машины и повторите сохранение.');
      }
      syncHistoryBtns(); return ok;
    } catch (err) {
      console.error(err);
      dirty = true;
      setText('saveState', 'Ошибка сохранения · повторите запись');
      showFileNote(false, 'Сохранение', 'Не удалось сохранить',
        'Проверьте свободное место и доступ к папке. Можно экспортировать JSON.');
      return false;
    }
  }
  ...
  function persist(silent, keepRev) {
    ...
    try {
      ...
      EditorData.save(pack);
      syncHistoryBtns();
      const slot = carIndex, serial = editSerial, c = pack.cars[slot];
      setText('saveState', 'Черновик сохранён · записываю файл…');
      if (c) {
        writeSlot(slot, c).then((ok) => {
          if (slot !== carIndex || serial !== editSerial) return;
          dirty = !ok;
          setText('saveState', ok ? 'Изменения записаны в файл' : 'Файл не записан · черновик сохранён локально');
          if (!silent) {
            setText('status', ok
              ? 'Настройки машины ' + EditorData.folderId(slot) + ' сохранены.'
              : 'Не удалось записать car.json. Проверьте доступ к папке и повторите сохранение.');
          }
        });
      } else if (!silent) {
  ...
  async function goToGame(query) {
    if (window.CarLightEditor) CarLightEditor.finish();
    flushCommit();
    if (!persist(true)) return;
    await Promise.all(Array.from(diskWrites.values()));
  ...
  function fillFields() {
    ...
    if (window.CarLightEditor) CarLightEditor.sync();
  }
  ...
  function selectCar(i) {
    ...
    if (i !== carIndex) {
      flushCommit();
      if (persistTimer || dirty) persist(true);
    }
    carIndex = i;
    editSerial++;
    dirty = diskStates.get(i) === 'failed';
    wheelIndex = 0;
    nitroIndex = 0;
    EditorView.clearSel();
    if (window.CarLightEditor && CarLightEditor.active()) EditorView.deselect();
    car();
    ...
    saveUiPref({car: i});
    syncHistoryBtns();
    const diskState = diskStates.get(i);
    setText('saveState', diskState === 'saving' ? 'Записываю файл машины…' : diskState === 'failed'
      ? 'Файл не записан · черновик сохранён локально' : diskState === 'saved' ? 'Изменения записаны в файл' : 'Настройки не изменены');
  }
  ...
  return {start, car, index: () => carIndex, dirty: () => { mark(); commitSoon(); },
    beginEdit, resumeSave, commitEdit: commitNow, refreshSelection: refreshSel, undo, redo, selectCar};
})();

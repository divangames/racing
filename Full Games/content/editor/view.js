...
const EditorView = (() => {
  ...
  /** Локальная точка автомобиля → координаты окна с учётом зума и ракурса. */
  function toScreen(p) {
    const r = canvas.getBoundingClientRect(), c = Math.cos(yaw), s = Math.sin(yaw);
    return {x: r.left + r.width / 2 + (p.x * c - p.y * s - cam.x) * cam.z,
      y: r.top + r.height / 2 + (p.x * s + p.y * c - cam.y) * cam.z};
  }
  ...
  function draw() {
    ...
    try {
      ...
      if (window.CarLightEditor) CarLightEditor.draw(ctx, car, z);
    } finally {
      ctx.restore();
    }
  }
  ...
  /** Автосвет опирается на тот же спрайт брони и запасной размер, что carHitHalf в гонке. */
  function lightBounds(car) {
    const scale = car.body.scale || 1, st = bodyStretch(car);
    const armor = imgChain(bodySrcList(car, getIndex(), true));
    const im = armor && armor.complete && armor.naturalWidth ? armor : imgChain(bodySrcList(car, getIndex(), false));
    if (im && im.complete && im.naturalWidth) {
      const s = 30 / Math.max(im.naturalWidth, im.naturalHeight) * scale;
      return {hw: im.naturalWidth * s * st.sx, hh: im.naturalHeight * s * st.sy};
    }
    return {hw: 27 * scale * st.sx, hh: 16 * scale * st.sy};
  }
  ...
  return {
    ...
    toWorld, toScreen, bodyDrawSize, lightBounds
  };
})();

...
const EditorData = (() => {
  ...
  const STATS = [
    ...
    {name:'«V8 ПЕРЕХВАТЧИК»',owner:null,price:8500,top:1.1,acc:1.07,crn:1.02,hp:125,col:'#2f7de0',col2:'#164a8c',hov:false,traits:['ГАТЛИНГ: лента пуль с перегревом','ПРИЦЕЛ: ульта замедляет всех вокруг','ФОРСАЖ: нитро 1.6x, +15% разгон']},
    {name:'«ШРЕДЕР»',owner:null,price:22000,top:1.16,acc:1.12,crn:1.12,hp:105,col:'#37c94f',col2:'#1c7a2e',hov:false,traits:['ЗУБ: пилы бьют с бортов','КЛЕТКА: три мины-паука назад','РЕГЕН: +2 корпуса в секунду']},
    {name:'«АЭРО-КЛИНОК»',owner:null,price:40000,top:1.24,acc:1.18,crn:1.28,hp:92,col:'#c9cdd6',col2:'#8a93a6',hov:true,traits:['ИМПУЛЬС: самонаводящаяся ракета','СБРОС ФАЗЫ: щиты и перезарядка','ХОВЕР: вне трассы без штрафа']},
    {name:'«ЧЁРНЫЙ МОЛОТ»',owner:null,price:68000,top:0.9,acc:0.85,crn:0.9,hp:180,col:'#1a1a1a',col2:'#8b0000',hov:false,traits:['ГАУБИЦА: снаряд с разрывом','МОНОЛИТ: броня и таран на 3 сек','ТАРАН: тройной удар, −50% от мин']},
    {name:'«ФАНТОМ»',owner:null,price:52000,top:1.3,acc:1.22,crn:1.25,hp:85,col:'#3a2a5c',col2:'#1a1028',hov:false,traits:['ПЛАЗМА: шар замедляет цель','МАСКИРОВКА: невидимость 3 сек','ФАЗА: проходит сквозь мины']},
    {name:'«УРАЛ»',owner:3,price:0,top:0.98,acc:0.97,crn:1.08,hp:120,col:'#7fb2ff',col2:'#2a4a7a',hov:false,traits:['БОРТ: шипы с кузова режут шины','ОТВАЛ: конус сбрасывает с носа','ВСЕДОХОД: меньше штраф бездорожья']},
  ...
  function mergeCar(i, saved) {
    ...
    const lights = RnRCarLights.normalize(saved.lights);
    if (lights) out.lights = lights;
    else delete out.lights;

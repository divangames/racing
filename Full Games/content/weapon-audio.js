...
function carWeaponVol() {
  ...
  if (!level) return 0;
  return window.DiVANEngine && DiVANEngine.audioMix && AU.ctx ? 1 : level;
}
...
function carWeaponField(listener, racer) {
  ...
  const pan = Math.max(-.82, Math.min(.82, dx / CAR_WEAPON_PAN));
...
function carWeaponPlayHtml(url, gain, pan) {
  const a = new Audio();
  ...
  if (mix) {
    mix.routeMedia(a, pan || 0);
    const release = () => mix.releaseMedia(a);
...
function carWeaponPlayFar(url, field, vol) {
  ...
  if (buf && buf !== 'bad' && buf !== 'html') {
    const src = AU.ctx.createBufferSource();
    src.buffer = buf;
    const g = AU.ctx.createGain();
    g.gain.value = Math.max(0, vol * CAR_WEAPON_FAR_VOL * field.gain);
  ...
  carWeaponPlayHtml(url, vol * CAR_WEAPON_FAR_VOL * field.gain, field.pan);
  return true;
}
...
function playCarWeapon(racer, kind) {
  if (!racer) return false;
  if (window.DiVANEngine && DiVANEngine.audioMix && !DiVANEngine.audioMix.live()) return true;
  ...
  if (carWeaponIsSelf(racer)) {
    carWeaponPlayHtml(carWeaponUrl(slot, slot.near), vol * .8, 0);
    return true;
  }
